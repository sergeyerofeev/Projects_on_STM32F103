...
bool N5110_Init() {
  N5110_Reset();
  if (!N5110_Write(PCD8544_CMD_FUNCTIONSET | PCD8544_BIT_EXTENDED, N5110_CMD))
    return false;
  if (!N5110_Write(PCD8544_CMD_SETVOP | PCD8544_DEFAULT_CONTRAST, N5110_CMD))
    return false;
  if (!N5110_Write(PCD8544_CMD_SETTEMP | PCD8544_DEFAULT_TEMP, N5110_CMD))
    return false;
  if (!N5110_Write(PCD8544_CMD_SETBIAS | PCD8544_DEFAULT_BIAS, N5110_CMD))
    return false;
  if (!N5110_Write(PCD8544_CMD_FUNCTIONSET, N5110_CMD))
    return false;
  if (!N5110_Write(PCD8544_CMD_DISPLAYCONTROL | PCD8544_BIT_DISPLAYNORMAL, N5110_CMD))
    return false;
...
bool N5110_SetContrast(uint8_t contrast) {
  ...
  if (!N5110_Write(PCD8544_CMD_FUNCTIONSET | PCD8544_BIT_EXTENDED, N5110_CMD))
    return false;
  if (!N5110_Write(PCD8544_CMD_SETVOP | contrast, N5110_CMD))
    return false;
  if (!N5110_Write(PCD8544_CMD_FUNCTIONSET, N5110_CMD))
    return false;
...
bool N5110_InvertDisplay(bool invert) {
  if (invert) {
    if (!N5110_Write(PCD8544_CMD_DISPLAYCONTROL | PCD8544_BIT_DISPLAYINVERTED, N5110_CMD))
      return false;
  } else {
    if (!N5110_Write(PCD8544_CMD_DISPLAYCONTROL | PCD8544_BIT_DISPLAYNORMAL, N5110_CMD))
      return false;
...
bool N5110_Update() {
  ...
  if (!N5110_Write(PCD8544_CMD_SETXADDR, N5110_CMD))
    return false;
  if (!N5110_Write(PCD8544_CMD_SETYADDR, N5110_CMD))
    return false;

  if (!N5110_BufferWrite(FrameBuffer, N5110_PIXEL))
    return false;
  HAL_Delay(10);
...
void N5110_SetPixel(uint8_t x, uint8_t y, bool set) {
  ...
  if (x < 0 || x >= N5110_MAX_WIDTH || y < 0 || y >= N5110_MAX_HEIGHT)
    return;
  ...
  if (fill) {
    for (int i = x1; i <= x2; i++) {
      for (int j = y1; j <= y2; j++) {
        ...
        if ((i == x1 && j == y1) || (i == x1 && j == y1 + 1) || (i == x1 + 1 && j == y1) || (i == x1 && j == y2) || (i == x1 && j == y2 - 1)
            || (i == x1 + 1 && j == y2) || (i == x2 && j == y1) || (i == x2 && j == y1 + 1) || (i == x2 - 1 && j == y1) || (i == x2 && j == y2)
            || (i == x2 && j == y2 - 1) || (i == x2 - 1 && j == y2)) {
          N5110_SetPixel(i, j, !set);
        } else {
          N5110_SetPixel(i, j, set);
        }
